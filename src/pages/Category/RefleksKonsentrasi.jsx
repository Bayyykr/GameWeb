...
import TangkapBintangGame from '../../components/game/TangkapBintangGame';
import KetukCepatGame from '../../components/game/KetukCepatGame';
import WarnaCocokGame from '../../components/game/WarnaCocokGame';
    ...
    const gameModes = [
        {
            ...
            description: 'Klik bintang-bintang yang jatuh dari langit!',
            icon: tangkapBintangIcon,
        ...
        },
        {
            id: 'ketuk-cepat',
            title: 'Ketuk Cepat',
            description: 'Ketuk monster yang muncul sebelum mereka sembunyi.',
            icon: ketukCepatIcon,
        ...
        },
        {
            id: 'warna-cocok',
            title: 'Warna Cocok',
            description: 'Pecahkan gelembung warna yang tepat!',
            icon: warnaCocokIcon,
        ...
        <div className="space-y-12">
            {!gameMode && (
                <>
                    ...
                    <header className="relative bg-indigo-950 rounded-[4rem] p-12 overflow-hidden border-8 border-amber-400 shadow-[0_20px_0_0_rgba(0,0,0,0.3)]">
                        ...
                        <div className="absolute -top-20 -right-20 w-80 h-80 bg-amber-500/20 rounded-full blur-[100px]"></div>
                        ...
                        <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
                            <div className="flex-shrink-0 animate-bounce-slow">
                                <img src={refleksKonsentrasiImg} alt="Refleks & Konsentrasi" className="w-56 h-56 md:w-72 md:h-72 object-contain drop-shadow-[0_20px_20px_rgba(0,0,0,0.5)]" />
                            </div>
                            <div className="text-center md:text-left">
                                ...
                                <h1 className="text-5xl md:text-8xl font-black text-white mb-4 tracking-tighter drop-shadow-[0_8px_0_#1e1b4b] uppercase italic leading-none">
                                    CEPAT <br /><span className="text-amber-400 drop-shadow-[0_8px_0_#92400e]">KILAT</span>
                                </h1>
                                <div className="inline-block bg-black/40 px-6 py-2 rounded-xl backdrop-blur-md border border-white/10 mt-2">
                                    <p className="text-amber-200 text-lg font-bold tracking-wide">
                                        Latih kecepatan jarimu dan fokus matamu dalam tantangan seru!
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <section>
                        <div className="flex items-center gap-4 mb-12">
                            ...
                            <h2 className="text-4xl font-black text-white tracking-widest uppercase drop-shadow-[0_4px_0_#1e1b4b]">ARENA REFLEKS</h2>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                            {gameModes.map((mode) => (
                                <div
                                    key={mode.id}
                                    className="group relative bg-indigo-950 border-4 border-white/10 rounded-[3rem] p-1 shadow-[0_12px_0_0_#4c0519] hover:-translate-y-2 transition-all duration-300 cursor-pointer overflow-hidden"
                                ...
                                >
                                    <div className="bg-indigo-900/40 rounded-[2.8rem] p-8 flex flex-col items-center text-center gap-6 border-4 border-transparent group-hover:border-amber-400/30 transition-colors h-full">
                                        <div className={`w-36 h-36 rounded-3xl bg-transparent flex items-center justify-center transform group-hover:rotate-6 transition-transform shrink-0`}>
            ...
            {gameMode && (
                typeof document !== 'undefined' ? createPortal(
                    ...
                    >
                        {/* Game Components */}
                        {gameMode === 'tangkap-bintang' && <TangkapBintangGame onExit={handleExitGame} />}
                        {gameMode === 'ketuk-cepat' && <KetukCepatGame onExit={handleExitGame} />}
                        {gameMode === 'warna-cocok' && <WarnaCocokGame onExit={handleExitGame} />}
                    </div>,
                    document.body
                ) : null
            )}
...
export default RefleksKonsentrasi;
